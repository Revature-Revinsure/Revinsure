...
public class CaimServiceTest extends RevinsureApplicationTests {
	...
	private final Claim fakeClaim = new Claim(-1, fakeUser, null, new Date(), 100, Status.PENDING, "test description");
	private final Claim updatedFakeClaim = new Claim(10, fakeUser, null, new Date(), 100, Status.PENDING, "test description");
	private final Claim nullClaimUser = new Claim(-1, null, null, new Date(), 100, Status.PENDING, "test description");
	private final Claim nullDate = new Claim(-1, fakeUser, null, null, 100, Status.PENDING, "test description");
	private final Claim nullStatus = new Claim(-1, fakeUser, null, new Date(), 100, null, "test description");
	private final Claim nullDescription = new Claim(-1, fakeUser, null, new Date(), 100, Status.PENDING, null);
	...
	public void testAddClaim() {
		...
		assertFalse(claimService.addClaim(null, fakeClaim));
	...
	@Test
	public void testGetUserClaims(User user) {
		
	}
	
	@Test
	public void testUpdateClaimStatus(Claim claim, Status newStatus) {
